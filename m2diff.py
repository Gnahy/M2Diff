...
from helping.utils import set_seed, mkdir, save_model, nmse, save_plots_pet_mri, load_model_weights, save_image, save_image_png, DDPM_Scheduler, tensor_to_pil_image, calculate_brain_mask
from helping.dataloaders import BiTaskPETDatasetMat_128
...
import sys
import csv
from improved_diffusion_PET_T1.script_util import create_bi_task_model_and_diffusion, bi_task_model_and_diffusion_defaults
import time
...
def setup(rank, world_size):
    ...
    os.environ['MASTER_PORT'] = '29501'  # Change this if needed
...
def parse_args():
    ...
    parser.add_argument('--train_data_path', type=str, default='../dataset/mat/train')
    parser.add_argument('--val_data_path', type=str, default='../dataset/mat/val')
    parser.add_argument('--test_data_path', type=str, default='../dataset/mat/test')
    parser.add_argument('--checkpoint_path', type=str, default='../iddpm_dir_PET_T1_adni/weights')
    parser.add_argument('--train_output_dir', type=str, default='../iddpm_dir_PET_T1_adni/results_train')
    parser.add_argument('--test_output_dir', type=str, default='../iddpm_dir_PET_T1_adni/results_test')
...
def validate(rank, model, val_dataloader, diffusion, criterion, ssim_metric, psnr_metric, output_dir, num_time_steps=1000):
    """
    ...
    with torch.no_grad():  # Disable gradient calculation
        for i, (lowdose, standarddose, mri) in enumerate(val_dataloader):
            if (i + 1) % 50 == 0:
                lowdose, standarddose, mri = lowdose.to(rank), standarddose.to(rank), mri.to(rank)
    
                # Sample from the diffusion model conditioned on lowdose and mri
                output = diffusion.p_sample_loop(
                    model,
                    standarddose.shape,
                    model_kwargs={"lowdose": lowdose, "mri": mri}
                )
                pet_pred = output['pet']
                mri_pred = output['mri']
                
                if torch.isnan(pet_pred).any() or torch.isinf(pet_pred).any() or torch.isnan(mri_pred).any() or torch.isinf(mri_pred).any():
                    print(f"NaN or Inf detected in batch {i}. Skipping this batch.")
                    continue
...
def train(rank, world_size, model, diffusion, train_dataloader, val_dataloader, args):

    setup(rank, world_size)
    # Create model and move it to the GPU corresponding to `rank`
    # scheduler = DDPM_Scheduler(num_time_steps=args.timesteps)
    model = model.to(rank)
    ddp_model = nn.parallel.DistributedDataParallel(model, device_ids=[rank], find_unused_parameters=True)
    ...
    ssim_metric = StructuralSimilarityIndexMeasure().to(rank)
    psnr_metric = PeakSignalNoiseRatio().to(rank)    
    ...
    for epoch in range(args.num_epochs):
        ...
        for lowdose, standarddose, mri in train_dataloader:
            ...
            t = torch.randint(0, diffusion.num_timesteps, (lowdose.shape[0],), device=rank)
            x_t = diffusion.q_sample(x_start=standarddose, t=t)
            loss_dict, bias_loss = diffusion.training_losses(model=ddp_model, x_start=standarddose, t=t, model_kwargs={"lowdose": lowdose, "mri": mri})
            # Extract losses
            pet_loss = loss_dict["pet_loss"].mean()
            mri_loss = loss_dict["mri_loss"].mean()
            
            if torch.isnan(pet_loss).any() or torch.isnan(mri_loss).any():
        ...
        avg_train_loss = running_train_loss/(len(train_dataloader)-skipped)
        ...
        if epoch == 0 or epoch >= 75:
            ...
            val_metrics = validate(rank, model, val_dataloader, diffusion, criterion, ssim_metric, psnr_metric, val_output_dir, args.timesteps)
            ...
            if validation_loss < best_val_loss:
               ...
               early_stopping_counter = 0
               save_model(model, args.checkpoint_path+'/iddpm_weights_best.pth')  # Save the best model
            else:
               early_stopping_counter += 1
               
            comb_ssim = val_metrics['ssim_pet'] + val_metrics['ssim_mri']
            if comb_ssim > best_ssim:
                best_ssim = comb_ssim
                save_model(model, args.checkpoint_path+'/iddpm_weights_ssim.pth')  # Save the best model
            
            save_model(model, args.checkpoint_path+'/iddpm_weights.pth')
...
def test(model, diffusion, test_loader, args):
    """
    ...
    load_model_weights(model, args.checkpoint_path + '/iddpm_weights_best.pth')
    ...
    for idx, (lowdose, standarddose, mri) in enumerate(test_loader):
        ...
        for i in range(len(pet_pred)):
            ...
            # Save images for PET
            pet_output_filename = os.path.join(args.test_output_dir, f'pet_test_batch_{idx}_image_{i}')
            ...
            # Save images for MRI
            mri_output_filename = os.path.join(args.test_output_dir, f'mri_test_batch_{idx}_image_{i}')
...
def main():
    ...
    train_dataset = BiTaskPETDatasetMat_128(lowdose_path, standarddose_path, mri_path)
    ...
    val_dataset = BiTaskPETDatasetMat_128(lowdose_path, standarddose_path, mri_path)
    ...
    test_dataset = BiTaskPETDatasetMat_128(lowdose_path, standarddose_path, mri_path)
    ...
    model_diffusion_params.update({
        "image_size": 128,  # Custom image size
    ...
    world_size = len(gpus)
    mp.spawn(train, args=(world_size, model, diffusion, train_loader, val_loader, args), nprocs=world_size, join=True)
    
    # Testing
    log_file_path = os.path.join(args.checkpoint_path, f'logs_{args.experiment_name}_best.txt')
    ...
    with open(log_file_path, 'w') as f:
        ...
        try:
            model = model.cuda()
            test(model, diffusion, test_loader, args)
        finally:
